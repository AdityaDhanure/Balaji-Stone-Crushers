...
class _CreateInvoiceSheetState extends ConsumerState<CreateInvoiceSheet> {
  ...
  void initState() {
    ...
    if (existing != null) {
      ...
      _taxPercent = existing.totalAmount > 0 && existing.taxAmount > 0
          ? (existing.taxAmount / (existing.totalAmount - existing.taxAmount)) * 100
          : 0;
      ...
      // Load existing items into the editable list
      ...
      Future.microtask(() async {
        ...
        // If invoice was loaded from list (items might be empty), fetch from API
        if (existing.items.isEmpty) {
          ...
          try {
            ...
            if (mounted) {
              setState(() {
                _items.clear();
                for (final raw in apiItems) {
                  final m = raw as Map<String, dynamic>;
                  _items.add({
                    'product_id': m['product_id'],
                    'product_name': m['product_name']?.toString() ?? m['description']?.toString() ?? 'Item',
                    'description': m['description']?.toString() ?? '',
                    'quantity': (m['quantity'] as num?)?.toDouble() ?? 0,
                    'selling_rate_per_unit': (m['selling_rate_per_unit'] as num?)?.toDouble() ?? 0,
      ...
      Future.microtask(() async {
        ref.read(customerProvider.notifier).loadCustomers();
        ref.read(productProvider.notifier).loadProducts();
        final number = await ref.read(billingProvider.notifier).getNextInvoiceNumber();
  ...
  double get _subtotal => _items.fold(0, (sum, item) => sum + ((item['amount'] as num?)?.toDouble() ?? 0));
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: Icon(
                    widget.existingInvoice != null ? Icons.edit_rounded : Icons.receipt_long_rounded,
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(child: _inputContainer(
                        child: TextField(
                          controller: _numberController, enabled: false,
                          style: const TextStyle(fontSize: 13, color: AppColors.textSecondary),
                          decoration: _inputDec('Invoice No.', Icons.confirmation_number_rounded),
                        ),
                      )),
                      ...
                      Expanded(child: _inputContainer(
                        child: TextField(
                          ...
                          decoration: _inputDec('Bill No. (Manual)', Icons.tag_rounded, hint: 'e.g. 001, B-45'),
                        ),
                      )),
                    ],
                  ),
                  ...
                  Row(
                    children: [
                      Expanded(child: _dateTile('Invoice Date', _invoiceDate, _selectInvoiceDate)),
                      const SizedBox(width: 10),
                        ...
                        child: Row(
                          children: [
                            Icon(Icons.person_search_rounded, size: 16,
                                color: _selectedCustomerName.isEmpty ? AppColors.textSecondary : AppColors.primary),
                            const SizedBox(width: 10),
                            Expanded(child: Text(
                              _selectedCustomerName.isEmpty ? 'Tap to select customer…' : _selectedCustomerName,
                              style: TextStyle(
                                  fontSize: 13,
                                  color: _selectedCustomerName.isEmpty ? AppColors.textSecondary : AppColors.textPrimary),
                            )),
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: const Row(mainAxisSize: MainAxisSize.min, children: [
                            ...
                            SizedBox(width: 4),
                            Text('Add Product', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.primary)),
                          ]),
                        ),
                      ),
                    ],
                  ...
                  if (_loadingItems)
                    Container(
                      ...
                      child: const Center(child: Row(mainAxisSize: MainAxisSize.min, children: [
                        ...
                        SizedBox(width: 10),
                        Text('Loading items...', style: TextStyle(fontSize: 12, color: AppColors.textSecondary)),
                      ])),
                    )
                    ...
                    Container(
                      ...
                      child: const Center(child: Column(children: [
                        ...
                        SizedBox(height: 8),
                        Text('No products added yet', style: TextStyle(color: AppColors.textSecondary, fontSize: 13)),
                      ])),
                    )
                  else
                    ...List.generate(_items.length, (index) {
                      ...
                      final rate = (item['selling_rate_per_unit'] as num?)?.toDouble() ?? 0;
                      ...
                      return Container(
                        ...
                        child: Row(children: [
                          Container(
                            ...
                            child: Center(child: Text('${index + 1}', style: const TextStyle(color: AppColors.primary, fontSize: 11, fontWeight: FontWeight.bold))),
                          ),
                          const SizedBox(width: 10),
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Text(item['product_name'] ?? item['description'] ?? 'Item',
                  ...
                  Row(
                    children: [
                      ...
                      SizedBox(
                        width: 72,
                        child: _inputContainer(child: TextField(
                      ...
                      const Padding(padding: EdgeInsets.symmetric(horizontal: 6), child: Text('%', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600))),
                    ],
                  ),
                  const SizedBox(height: 12),
                  Container(
                    ...
                    child: Column(children: [
                      ...
                      if (_taxPercent > 0) ...[const SizedBox(height: 6), _amtRow('GST (${_taxPercent.toStringAsFixed(1)}%)', _taxAmount, fmt)],
                      const Divider(height: 16, color: AppColors.border),
                  ...
                  _label('Notes'),
                  const SizedBox(height: 6),
                  _inputContainer(child: TextField(
                    controller: _notesController, maxLines: 2,
                    style: const TextStyle(fontSize: 13),
                    decoration: _inputDec('Optional notes...', Icons.notes_rounded),
                  )),
            ...
            child: SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: _isSubmitting
                    ...
                    : Text(
                        widget.existingInvoice != null ? 'UPDATE INVOICE' : 'CREATE INVOICE',
  ...
  Future<void> _submit() async {
    ...
    try {
      await widget.onSave({
        ...
        'bill_no': _billNoController.text.isEmpty ? null : _billNoController.text,
  ...
  Widget _dateTile(String label, DateTime? date, VoidCallback onTap, {String? placeholder}) =>
    GestureDetector(
      onTap: onTap,
      child: _inputContainer(child: Padding(
        padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 13),
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(
              date == null ? (placeholder ?? 'Select') : DateFormat('dd MMM yyyy').format(date),
              style: TextStyle(
                  fontSize: 12,
  ...
  Widget _amtRow(String label, double amount, NumberFormat fmt, {bool bold = false}) => Padding(
    padding: const EdgeInsets.symmetric(vertical: 3),
    child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      Text(label, style: TextStyle(
          fontSize: bold ? 14 : 13, fontWeight: bold ? FontWeight.bold : FontWeight.normal,
          color: AppColors.textSecondary)),
      Text('₹${fmt.format(amount)}', style: TextStyle(
          fontSize: bold ? 16 : 13, fontWeight: bold ? FontWeight.bold : FontWeight.normal,
  ...
  void _selectInvoiceDate() async {
    final picked = await showDatePicker(context: context, initialDate: _invoiceDate, firstDate: DateTime(2020), lastDate: billingTodayIstDate().add(const Duration(days: 365)));
    if (picked != null) setState(() => _invoiceDate = picked);
  }

  void _selectDueDate() async {
    final picked = await showDatePicker(context: context, initialDate: _dueDate ?? _invoiceDate.add(const Duration(days: 30)), firstDate: _invoiceDate, lastDate: billingTodayIstDate().add(const Duration(days: 365)));
        ...
        child: Column(
          children: [
            ...
            TextField(
              ...
              decoration: InputDecoration(hintText: 'Search by name, phone, or code...', prefixIcon: const Icon(Icons.search), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))),
            ...
            Expanded(
              child: Consumer(
                builder: (context, ref, _) {
                  ...
                  final customers = searchController.text.isEmpty ? customerState.customers : customerState.searchResults;
                  if (customers.isEmpty) {
                    return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.search_off, size: 64, color: AppColors.textSecondary), const SizedBox(height: 16), const Text('No customers found')]));
                  }
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return ListTile(
                        leading: CircleAvatar(backgroundColor: AppColors.primary.withValues(alpha: 0.1), child: Text(customer.name[0].toUpperCase())),
                        title: Text(customer.name),
                        subtitle: Text('${customer.customerCode} â€¢ ${customer.phone ?? "No phone"}'),
                        onTap: () {
                          setState(() { _selectedCustomerId = customer.id; _selectedCustomerName = customer.name; });
  ...
  void _showAddProductDialog() {
    showModalBottomSheet(
      ...
      builder: (bottomSheetContext) => AddProductSheet(
        onAdd: (productData) {
          setState(() { _items.add(productData); });
          Navigator.pop(bottomSheetContext);
        },
      ),
    );
...
class _AddProductSheetState extends ConsumerState<AddProductSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    final fmt = NumberFormat('#,##,###');
    final canAdd = _selectedProductId != null &&
          ...
          children: [
            ...
            Row(children: [
              Container(
                ...
                child: const Icon(Icons.inventory_2_rounded, color: AppColors.accent, size: 18),
              ),
              const SizedBox(width: 10),
              ...
              child: Container(
                ...
                child: Row(children: [
                  Icon(Icons.inventory_2_outlined, size: 16,
                      color: _selectedProductId != null ? AppColors.primary : AppColors.textSecondary),
                  const SizedBox(width: 10),
                  Expanded(child: Text(
                    _selectedProductName.isEmpty ? 'Tap to select product' : _selectedProductName,
                    style: TextStyle(
                        fontSize: 13,
                        color: _selectedProductName.isEmpty ? AppColors.textSecondary : AppColors.textPrimary,
                        fontWeight: _selectedProductName.isEmpty ? FontWeight.normal : FontWeight.w600),
                  )),
                  const Icon(Icons.arrow_drop_down_rounded, color: AppColors.textSecondary),
                ]),
              ),
            ...
            Row(
              children: [
                Expanded(child: _numField(_quantityController, 'Quantity (brass)', Icons.scale_rounded)),
                const SizedBox(width: 10),
                Expanded(child: _numField(_rateController, 'Rate / brass', Icons.currency_rupee_rounded,
            ...
            Container(
              ...
              child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                const Text('Calculated Amount', style: TextStyle(fontSize: 13, color: AppColors.textSecondary)),
            ...
            Container(
              ...
              child: TextField(
                ...
                decoration: const InputDecoration(
                  ...
                  prefixIcon: Icon(Icons.notes_rounded, size: 16, color: AppColors.textSecondary),
                  border: InputBorder.none,
            ...
            SizedBox(
              width: double.infinity,
              child: ElevatedButton(
                onPressed: canAdd ? () => widget.onAdd({
                  'product_id': _selectedProductId,
                  'product_name': _selectedProductName,
                  'description': _descriptionController.text.isEmpty ? _selectedProductName : _descriptionController.text,
                  'quantity': double.tryParse(_quantityController.text) ?? 0,
                  'selling_rate_per_unit': double.tryParse(_rateController.text) ?? 0,
                  'amount': _amount,
                }) : null,
                style: ElevatedButton.styleFrom(
                      ...
                      itemBuilder: (_, i) {
                        ...
                        return ListTile(
                          ...
                          leading: Container(
                            ...
                            child: Center(child: Text('${p.sizeMm ?? "?"}mm',
                          ...
                          subtitle: Text('${p.productCode} | \u20b9${p.currentRate.toStringAsFixed(0)}/brass',
                              style: const TextStyle(fontSize: 11, color: AppColors.textSecondary)),
                          onTap: () {
                            setState(() {
                              ...
                              if (_rateController.text.isEmpty) {
                                _rateController.text = p.currentRate.toStringAsFixed(0);
                              }
                            });
                            Navigator.pop(ctx);
                          },
